...
public class BiliDanmakuParser extends BaseDanmakuParser {
    ...
    private static final String TAG = "BiliDanmakuParser";

    private Config config;
    private Context context;
    private File danmaku;
    private DanmakuRule[] rules = null;

    private float mDispScaleX;
    private float mDispScaleY;

    public BiliDanmakuParser(Context context, File danmaku) {
        config = new Config(context);
        this.context = context;
        this.danmaku = danmaku;
    }

    public void loadRules() throws ParserConfigurationException, SAXException, IOException {
        rules = null;
        ((MainActivity) context).setProgressBarText("读取屏蔽列表...\n点击取消按钮可停止加载。");
        File dir = context.getFilesDir();
        SAXParserFactory spf = SAXParserFactory.newInstance();
        SAXParser saxParser = spf.newSAXParser();
        DanmakuRulesHandler handler = new DanmakuRulesHandler();
        // SAX的bug：如果注释中有换行符，则不能被正确处理。
        String file = DanmakuRulesHelper.readFile(new File(config.getDanmakuRulesPath()));
        DanmakuRulesHelper.writeFile(file.replaceAll("<!--.*?-->", ""), new File(dir + File.separator + "rules.xml"));
        saxParser.parse(new FileInputStream(new File(dir + File.separator + "rules.xml")), handler);
        rules = handler.getRules().toArray(new DanmakuRule[0]);
    }
    ...
    public class XmlContentHandler extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) {
            ...
            if (tagName.equals("d")) {
                ...
                if (values.length > 0) {
                    ...
                    int color = (int) ((0x00000000ff000000 | Long.parseLong(values[3]))); // 颜色
                    // int poolType = Integer.parseInt(values[5]); // 弹幕池类型（忽略
                    item = mContext.mDanmakuFactory.createDanmaku(type, mContext);
                    if (item != null) {
                        // 感谢zou-xian-liang 在issue #317 给出的解决办法
        ...
        public void endElement(String uri, String localName, String qName) {
            ...
            ((MainActivity) context).setProgressBarText("已读取弹幕：" + count1 + " ，屏蔽弹幕：" + count2 + "\n点击取消按钮可停止加载。");
        }

        @Override
        public void characters(char[] ch, int start, int length) {
            if (((MainActivity) context).getDanmakuView() == null) {
                return;
            }
            if (item != null) {
                ...
                // 以下为屏蔽弹幕处理逻辑
                if (config.getDanmakuRulesEnabled() && rules != null) {
                    int mode = config.getDanmakuDisplayMode();
                    String str = (String) item.text;
                    if (mode == MODE_NORMAL) {
                        for (DanmakuRule rule : rules) {
                            if (rule.getType() == DanmakuRule.TYPE_TEXT) {
                                ...
                                Pattern pattern = Pattern.compile(rule.getContent());
                                ...
                                if (item.userHash.toLowerCase().equals(rule.getContent().toLowerCase())) {
                                    item = null;
                                    count2++;
                                    return;
                                }
